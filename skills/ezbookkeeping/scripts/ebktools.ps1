...
# A command-line tool for calling AbelFinance APIs
...
$API_CONFIGS = @(
    ...
    @{
        Name = "server-version"
        Description = "Retrieve AbelFinance server version information"
        Method = "GET"
...
function Show-Help {
    ...
    Write-Host ""
    Write-Host "A command-line tool for calling AbelFinance APIs"
    Write-Host ""
    Write-Host "Usage:"
    ...
    Write-Host "    EBKTOOL_SERVER_BASEURL      AbelFinance server base URL (e.g., http://localhost:8080)"
    Write-Host "    EBKTOOL_TOKEN               AbelFinance API token"
    Write-Host ""
...
function Invoke-Api {
    ...
    if (-not $serverBaseUrl) {
        ...
        Write-Host "Please set it to your AbelFinance server base URL (e.g., http://localhost:8080)"
        exit 1
    }
